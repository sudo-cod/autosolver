# ALGORITHM: ILP two-phase (OR-Tools CP-SAT + PuLP/CBC) with scarcity-aware greedy fallback
...
def solve(input_text: str) -> list:
    ...
    # Scarcity-aware greedy fallback
    # Key insight: when couriers are scarce relative to tasks, we need to prioritize
    # tasks with FEWER options (higher scarcity) to avoid leaving them uncovered
    
    # Calculate scarcity for each task (fewer candidates = more scarce)
    task_scarcity = {}
    for t in all_tasks:
        task_scarcity[t] = len(task_to_cands.get(t, []))
    
    # Calculate scarcity for each courier (fewer tasks they can cover = more scarce)
    courier_scarcity = {}
    for cid in all_couriers:
        tasks_covered = set()
        for i in courier_to_cands.get(cid, []):
            for t in candidates[i][4]:
                tasks_covered.add(t)
        courier_scarcity[cid] = len(tasks_covered)
    
    def scarcity_aware_score(item):
        ...
        # Secondary: scarcity of tasks (higher scarcity = should be assigned earlier)
        # Use max scarcity among tasks in this candidate
        max_scarcity = max(task_scarcity.get(t, 0) for t in tids)
        # Invert: lower scarcity value = more scarce = higher priority
        scarcity_priority = -1.0 / max_scarcity if max_scarcity > 0 else 0
        
        # Tertiary: willingness (higher is better)
        return (score_per_task, scarcity_priority, -will)
    
    candidates_sorted = sorted(candidates, key=scarcity_aware_score)
