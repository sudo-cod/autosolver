# ALGORITHM: ILP two-phase (PuLP/CBC + OR-Tools CP-SAT) with regret-based greedy fallback
...
def solve(input_text: str) -> list:
    ...
    # For each task, find the best and second-best assignment score
    # Tasks with high regret (big gap between best and second-best) get priority
    
    task_best = {}  # task -> (best_score, cand_idx)
    task_second = {}  # task -> second_best_score
    ...
    # Sort candidates by a combined score considering regret of their tasks
    def cand_priority(item):
        score, tils, cid, will, tids = item
        n_tasks = len(tids)
        score_per_task = score / n_tasks if n_tasks > 0 else float('inf')
        # Average regret of tasks in this candidate
        avg_regret = sum(task_regret.get(t, 0) for t in tids) / n_tasks if n_tasks > 0 else 0
        # Higher regret = should be assigned earlier
        # Lower score_per_task = better
        return (-avg_regret, score_per_task, score)
    
    candidates_sorted = sorted(enumerate(candidates), key=lambda x: cand_priority(x[1]))
    
    assigned_couriers = set()
    assigned_tasks = set()
    result = []

    for orig_idx, (score, tils, cid, will, tids) in candidates_sorted:
    ...
    # Second pass: try to fill any remaining gaps
