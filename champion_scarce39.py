...
def ilp_primary(bundle_records, all_tasks, time_limit,
                p_fail_bias=P_FAIL, scarce_mode=False, allow_drop=False):
    try:
        import pulp
    except ImportError:
        return None
    ...
    for ts, lst in bundle_records.items():
        ...
        if not scarce_mode and n_in == 2:
            continue
        if scarce_mode and not allow_drop and n_in != 2:
            continue
        for s, w, c in lst:
            if w < 0.005:
                continue
    ...
    if allow_drop:
        prob += pulp.lpSum((cost - 100.0 * len(ti)) * x[(ts, c)]
                           for ts, c, cost, ti in cands)
    else:
